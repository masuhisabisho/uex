...
''' <returns>単語・フォントサイズを格納した配列を返す</returns>	
	...
	Public function WordPreparer(fontSize As String, mainTxt As ArrayList) As ArrayList
		...
		Dim registChkFlg As Boolean = False
			...
			For i As Integer = 0  To lineCounter Step 1
				...
				Select Case selector
					Case 0	'上																'END: 上の場合の文字ピッチを計算
						...
						If PointDiffChecker(storageWord(i)) = True Then
							...
							If storageWord(i)(0)(0) <> "" Then							'文字あり
								...
								Dim fontSize() As Single = GetMaxWord(Wc.optWord("Common_Font"),,storageWord(i),)
...
'								wordDetail = {"", "", "", ""}							下記に変更　2013/8/4 out 5 lines above mb
								storageWord(0)(2) = startYPos
								storageWord(0)(3) = startXPos
								Continue For
							End If
							'End If
...
'******************************未使用↑******************************
					...
					Case 2	'天地
						...
						If PointDiffChecker(storageWord(i)) = True Then							'★★全て同じフォントサイズの時★★
							newStartYPos = CheckNewYPos(CSng(mainTxt(i)("tbl_txt_newypos")))
							If newStartYPos <> 0 Then
								startYPos = newStartYPos'										y軸イレギュラースタートの確認
							End If
...
''' <param name="word">ArrayList 単語・フォントサイズ・xy軸位置の配列</param>
...
''' <returns>True = 全て同じ False = 異なるものあり</returns>
	...
	Public Function PointDiffChecker(word As ArrayList) As Boolean
